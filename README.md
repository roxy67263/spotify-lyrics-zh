...
- 如果有設定 `DEEPSEEK_API_KEY`，會優先使用 DeepSeek 做低成本歌詞翻譯。
- 如果有設定 `OPENAI_API_KEY`，翻譯會使用 OpenAI 以歌詞上下文做自然繁中翻譯。
- 如果沒有設定 `OPENAI_API_KEY`，會退回免金鑰的 Google Translate 網路端點，適合個人 MVP。
